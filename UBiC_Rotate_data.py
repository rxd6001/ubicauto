...
def krw_markets():
    ...
    if isinstance(contents, list):                          # contents 가 리스트일 경우 전체종목 및 ** 유의종목 ** 제외한 원화마켓 종목만 호출 
        krw_markets1 = [x['market'] for x in contents if x['market'].startswith('KRW')]     # 전체종목
        krw_markets2 = [x['market'] for x in contents if x['market_warning'] not in 'CAUTION' and x['market'].startswith('KRW')]    # 유의종목 제외

        with open('/root/UBiCauto/data/krw_markets.pickle', 'wb') as fw:        # 원화마켓 종목의 데이터를 파일로 저장
            pickle.dump(krw_markets1, fw)     

        with open('/root/UBiCauto/data/krw_markets_e.pickle', 'wb') as fw:        # 원화마켓 종목의 데이터를 파일로 저장
            pickle.dump(krw_markets2, fw)          

def km1():
    with open('/root/UBiCauto/data/krw_markets.pickle', 'rb') as fr2:       # 전체종목 파일로 불러 옴
        km_all = pickle.load(fr2)
    return km_all

km = km1()                  # 원화마켓 전 종목 변수로 저장

def km2():
    with open('/root/UBiCauto/data/krw_markets_e.pickle', 'rb') as fr2:       # 유의 제외종목 파일로 불러 옴
        km_exp = pickle.load(fr2)
    return km_exp

km_e = km2()                  # 원화마켓 유의 제외종목 변수로 저장
...
# 원화종목을 한번에 모든종목 가져올 경우 (전체종목 및 유의 제외종목)
...
def cp4():                                          # 원화종목이 100개 이상으로 나눠서 호출 후 합침
    try:
        ...
        with open('/root/UBiCauto/data/cp.pickle', 'wb') as fw3:     # 원화마켓 종목의 현재가 데이터를 파일로 저장
            pickle.dump(cp1, fw3)
        cp2 = pyupbit.get_current_price(km_e)       # 종목별 현재가를 호출(유의 제외종목)
        with open('/root/UBiCauto/data/cp_e.pickle', 'wb') as fw3:     # 원화마켓 종목의 현재가 ㄷ이터를 파일로 저장
            pickle.dump(cp2, fw3)
