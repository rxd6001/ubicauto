...
# 종목당 30,000 원 기준 : 1차 추매 90,000 / 2차 추매 180,000 / 3차 추금 540,000 * 30, 16,200,000 (40, 21,600,000)
...
# 종목당 5,500 원 기준 : 1차 추매 16,500 / 2차 추매 원3,000 / 3차 추매 99,000
# 종목당 5,000 원 기준 : 1차 추매 15,000 / 2차 추매 30,000 / 3차 추매 90,000
# 총 투자금 600만원 + 10만원 / 투자비율(3%) / 25종목 / 매수금액 약 8,200원
# 6,000,000 :0.75, 10 ea, 5,000 / 1,3, 15 ea
# 21.11.9일 bp,
# 22.01.01 : 114만원 입금, 총 원금 724만원 / 22.01.19 : 20만원 출금 (704만원) / 22.01.28 : 5만원 입금 (709만원)
# 22.08.16 : 200만원 출금 (509만원) / 22.10.20 : 20만원 출금 (489만원)
# 투자원금 709만원
""" =============================== 로그인 파일 변수 설정 ==================================== """
...
b_money = 8000                  # 0.75 % 시 7100원 부터 매수가능함  10,000 초기 매수 금액 설정 (금액이 적어지면 신규매수 시 add_num에 못들어가서 1차 추매 시 "nonetype error")

# ========================= b_money 계산 ======================== #
bp = 3.7 # 2.8  # 2.6 #6.5  # 2.6  # 2.173                         # 보유금액의 % 매수(개당 매수금액 계산필요)
b_count = 35 # 30                     # 보유 수량 제한 설정
blow_money = 8000               # 최저 매수금액
# ============================================================== #
...
ap = 8 #7 #6                                           # 추매 횟수 제한, 4번 까지
u_p = 4                                           # 추매 변수 (현재가 < 50이의 4% 이하 시 추매 경우, % 설정)

""" --------------- """
l_money = 280000                                 # 추매 5회까지 가능 / 금액 제한 (종목당 보유금액( 보유금액의 2배 까지 증가 가능)
""" --------------- """
...
# ag4 = 0.75                   # 추매 금액 (종목 보유금액의 0.75배)
# ag5 = 3                     # 추매 금액 (종목 보유금액의 3배)


""" 기존 - 매수금액 : 5100원 추매시점 3,5,7,10/ ap=5 / l_money= b_money * 7/ b_money=5100 """

# ag1 = 1 # 0.5                     # 추매 금액 (종목 보유금액의 0.7배)
# ag2 = 1 # 0.35                     # 추매 금액 (종목 보유금액의 0.8배)
# ag3 = 1 # 0.3                     # 추매 금액 (종목 보유금액의 0.9배)
# ag4 = 1 # 0.5                       # 추매 금액 (종목 보유금액의 1배)
# ag5 = 1 # 0.5                     # 추매 금액 (종목 보유금액의 1.5배)
# ag6 = 1 # 0.5                       # 추매 금액 (종목 보유금액의 2배)
# ag7 = 1 # 0.5                       # 추매 금액 (종목 보유금액의 2배)
# ag8 = 1 # 0.5                       # 추매 금액 (종목 보유금액의 2배)

# """ 현재 - 매수금액 : 15000원 추매시점 3,5,7,10/ ap=6 / l_money= b_money * 5/ b_money=15000 """
# ag1 = 0.5                      # 추매 금액 (종목 보유금액의 배)
# ag2 = 0.3                      # 추매 금액 (종목 보유금액의배)
# ag3 = 0.5                      # 추매 금액 (종목 보유금액의 배)
# ag4 = 1                      # 추매 금액 (종목 보유금액의 배)
# ag5 = 1                      # 추매 금액 (종목 보유금액의 배)
# ag6 = 1                        # 추매 금액 (종목 보유금액의 배)    *****
# ag7 = 0.5                        # 추매 금액 (종목 보유금액의 배)
# ag8 = 0.5                      # 추매 금액 (종목 보유금액의 배)

""" --------------------------------------------------------------------------------------------- """

""" 현재 - 매수금액 : 15000원 추매시점 1,2,3,5,7,10/ ap=6 / l_money= b_money * 5/ b_money=15000 / 20210821"""

ag1 = 1                      # 추매 금액 (종목 보유금액의 배)
ag2 = 0.5                      # 추매 금액 (종목 보유금액의배)
ag3 = 0.5                      # 추매 금액 (종목 보유금액의 배)
ag4 = 0.3                      # 추매 금액 (종목 보유금액의 배)
ag5 = 0.5                      # 추매 금액 (종목 보유금액의 배)
ag6 = 1                        # 추매 금액 (종목 보유금액의 배)    *****
...
with open('/root/UBiCauto/data/config_buy3.pickle', 'wb') as fbuy:
    # ----- 매수 변수 ----- #
    ...
    pickle.dump(p1050g, fbuy); pickle.dump(ext_b, fbuy); pickle.dump(ofn3, fbuy)
    pickle.dump(blow_money, fbuy); pickle.dump(nw, fbuy); pickle.dump(yg, fbuy)
    pickle.dump(npct, fbuy)

with open('/root/UBiCauto/data/config_bcode3.pickle', 'wb') as fbuy:
    pickle.dump(code, fbuy)


with open('/root/UBiCauto/data/config_sell3.pickle', 'wb') as fsell:
    # ----- 매도 변수 ----- #
    ...
    pickle.dump(abptime, fsell); pickle.dump(ext_s, fsell); pickle.dump(ofn3, fsell)
    pickle.dump(ask_ug, fsell); pickle.dump(adsp, fsell); pickle.dump(blow_money, fsell)

# with open('/root/UBiCauto/data/config_scode3.pickle', 'wb') as fsell:
