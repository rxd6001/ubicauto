...
from UBiC_Search_def import buy_avr_price_5, buy_avr_price_10, buy_avr_price_20, buy_avr_price_30, buy_avr_price_40, \
    buy_avr_price_50, buy_avr_price_60, buy_avr_price_70, buy_avr_price_80, buy_avr_price_90, ma5_under, s1_bull_m1, \
        b2_decline_m1, get_orderi, abp_check, get_tick_s, t_c, ex_item, perct, adn_insert, myasset, askper, add_rwinit
from UBiC_Rotate_data import gb, upbit3
from datetime import datetime, timedelta

""" ======= 추매파일이 경로에 있으면 불러오고 없으면 통과 ======== """
if os.path.isfile('/root/UBiCauto/data/add_num3.pickle'):
    with open('/root/UBiCauto/data/add_num3.pickle', 'rb') as fr:
        add_num = pickle.load(fr)

now = datetime.now()    # 현재시간 호출
rwnum = 3           # add_num에 필요한 변수
ex_num = 3          # add_num 종목제외 시 필요(ex_item(bl, ex_num))
...
while True:
    ...
    with open('/root/UBiCauto/data/ext_item3.pickle', 'rb') as fset:
    ...
    ofn3 = '/root/UBiCauto/Acct/upbit3.txt'

    t_tp = []                                             # 평가손익(리스트)

    try:
        bl, abp1, tb1 = gb(upbit3)                              # 보유종목명, 평단, 보유수량 호출                        
        tsum, tbidm, tbidp, summ = myasset(upbit3)               # 자산정보 한 번에 호출
        summ = round(summ, 1)
    ...
    try:
        ...
        for e in bl:                                            # 자산('bl') 리스트를 하나씩 'e'로 가져 옴
            if abp_check(e, abptime, ofn3):                           # 매수 평단 선택 (체크)
                abp = abp_check(e, abptime, ofn3)                     # 매수평단 최근 매수완료 정보에서 가져 옴
            else:
            ...
            if (e in cu_p):                                     # 한 종목에 대한 현재가 추출
                ...
                with open('/root/UBiCauto/data/set_cfg3.pickle', 'rb') as fset:
                    ubic_conf = pickle.load(fset)
                b_money = ubic_conf['b_money']          # set_cfg3.pickle에서 b_money 값을 호출

                if t_money <= 5000:                  # 매도 후 잔금이 5000원 이하 남을 시 추매
                    bid_money = (b_money + 200) - t_money
                    upbit3.buy_market_order(e, bid_money)   # 현매수금액과 차액만큼 매수
                    ...
                    # 미적용
                        ...
                        #     if 5000 < g_money:
                        #         upbit3.buy_market_order(e, g_money)
                        #     elif g_money <= 5000:
                ...
                if sell_cont == 0:                                      # 전체 매도 제외, 전체 추매 제외
                    ...
                    if (sell_cont == 1) or (sell_cont == 2):            # 매도기능
                        if s1_bm1 and (s1_bm2 < price):                 # bulling + 매도기준 만족 시
                            ...
                            if e in ext_s:
                                ...
                                a_order = upbit3.sell_limit_order(e, price, tb)       # '지정가 < 현재가' 조건 시 현재가 매도 실행

                                # 미적용
                                    # print(j, "판매완료")                        
                                    # upbit3.sell_market_order(e, tb)  # '종목' 을 시장가로 매도
                                    # a_order = upbit3.sell_limit_order(e, a_o, tb)       # '지정가 < 현재가' 조건 시 지정가 매도 실행
                    ...
                    if (sell_cont == 1) or (sell_cont == 3):          # 추매기능
                        if e in ext_a:
                            ...
                            if e not in add_num :
                                cbid = get_orderi('done', 'bid', 'i', ofn3)         # 매수완료 정보 가져오기
                            ...
                            if b2_bm1 and (price < b2_bm2):           # decline + 추매지연기준 2    
                                ...
                                upbit3.buy_market_order(e, ag_price1)   # 1, 20,000 (1)
                                ...
                                upbit3.buy_market_order(e, ag_price2)   # 1, 40,000 (2)
                                with open('/root/UBiCauto/find_s1.log','a') as fw:
                                ...
                                upbit3.buy_market_order(e, ag_price3)   # 1, 80,000 (3)
                                with open('/root/UBiCauto/find_s1.log','a') as fw:
                                ...
                                upbit3.buy_market_order(e, ag_price4)   # 1, 160,000 (4)
                                with open('/root/UBiCauto/find_s1.log','a') as fw:
                                ...
                                upbit3.buy_market_order(e, ag_price5)   # 1배 추매 1회
                                with open('/root/UBiCauto/find_s1.log','a') as fw:
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    upbit3.buy_market_order(e, ag_price1)   # 0.7배 추매
                                    ...
                                    upbit3.buy_market_order(e, ag_price1)
                                    if 5000 <= addprice_1:
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    ...
                                    upbit3.buy_market_order(e, ag_price2)   # 1배 추매
                                    if 5000 <= addprice_2:
                                        add_rwav5(an, e, rwnum)

                                elif (buy_goal10[1] < abp) and (price < buy_goal20[0]) and (t_money < l_money):
                                
                                    upbit3.buy_market_order(e, ag_price2)
                                    if 5000 <= addprice_2:
                                        add_rwav10(an, e, rwnum)
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    ...
                                    upbit3.buy_market_order(e, ag_price3)   # 1배 추매
                                    if 5000 <= addprice_3:
                                        add_rwav5(an, e, rwnum)

                                elif (buy_goal10[1] < abp) and (price < buy_goal20[0]) and (t_money < l_money):
                                
                                    upbit3.buy_market_order(e, ag_price3)
                                    if 5000 <= addprice_3:
                                        add_rwav10(an, e, rwnum)
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    ...
                                    upbit3.buy_market_order(e, ag_price4)   # ** 1.5배 추매 **
                                    if 5000 <= addprice_4:
                                        add_rwav5(an, e, rwnum)
                                    ...
                                    upbit3.buy_market_order(e, ag_price4)
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    ...
                                    upbit3.buy_market_order(e, ag_price5)   # ** 2배 추매 **
                                    if 5000 <= addprice_5:
                                        add_rwav5(an, e, rwnum)
                                    ...
                                    upbit3.buy_market_order(e, ag_price5)
                                    if 5000 <= addprice_5:
                                        add_rwav50(an, e, rwnum)
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    ...
                                    upbit3.buy_market_order(e, ag_price6)   # 1배 추매 **
                                    if 5000 <= addprice_6:
                                        add_rwav5(an, e, rwnum)
                                    ...
                                    upbit3.buy_market_order(e, ag_price6)
                                    if 5000 <= addprice_6:
                                        add_rwav60(an, e, rwnum)
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    ...
                                    upbit3.buy_market_order(e, ag_price7)   # 1배 추매 **
                                    if 5000 <= addprice_7:
                                        add_rwav5(an, e, rwnum)
                                    ...
                                    upbit3.buy_market_order(e, ag_price7)
                                    if 5000 <= addprice_7:
                                        add_rwav70(an, e, rwnum)
                                ...
                                if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                    ...
                                    upbit3.buy_market_order(e, ag_price8)   # 1배 추매 **
                                    if 5000 <= addprice_8:
                                        add_rwav5(an, e, rwnum)
                                    ...
                                    upbit3.buy_market_order(e, ag_price8)
                                    if 5000 <= addprice_8:
                                        add_rwav80(an, e, rwnum)
        ...
        with open('/root/UBiCauto/data/t_tp3.pickle', 'wb') as fw:    # 보유종목 평가손익(list 로 저장)
            pickle.dump(t_tp, fw)
    except Exception as s1:
        print('e-time:', datetime.now())
        print('s1_매도및추매 프로세스', s1)
    sum_tp = sum(t_tp)                  # 총 보유 평가손익 계산금액
    ask_p = askper(upbit3, 3)                   # 매도기준 조정 및 저장
    ...
    try:    # 매도대기 취소
        ask_w_i = get_orderi('wait', 'ask', 'i', ofn3)                                # 매도대기 목록 정보 리스트
        ask_w_list = get_orderi('wait', 'ask', 'n', ofn3)                             # 매도대기 종목 리스트
        ...
        for an in range(len(ask_w_list)):                               # list 에서 매도대기 종목(dict) 항목 추출
            ...
            if (uuids_an in ext_s) or ("KRW-BTT" == uuids_an):             # 매도제외 종목에 대기종목이 있을 경우 또는 KRW-BTT 
                ...
                if (mt <= datetime.now()):                                  # 지정시간이 지났는지, 자산목록에 종목이 있는지 확인 후
                    b_cancel = upbit3.cancel_order(uuids_aid)               # 매도 대기 취소
    except Exception as s3:
        print('e-time:', datetime.now())
        print('s3_매도대기', s3)


    if os.path.isfile('/root/UBiCauto/data/add_num3.pickle') and os.path.getsize('/root/UBiCauto/data/add_num3.pickle') > 5:
        os.system('clear')
