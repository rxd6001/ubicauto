...
""" ======================================================================== """
# from UBiC_Rotate_data import cp1, cp2, gb, upbit1
...
while True:
    ...
    with open('/root/UBiCauto/data/ext_item1.pickle', 'rb') as feit:
    ...
    # mybal = round(myasset(upbit1)[3], 1)          # 예전 코드

    mybal = upbit1.get_balance_t()                  # 매수가능금액
    ...
    with open('/root/UBiCauto/data/set_cfg1.pickle', 'wb') as fw:
    ...
    try:    # 매수대기 종목명 가져오기
        ...
        bid_w_list = get_orderi('wait', 'bid', 'n', ofn1)            # 매수 대기 종목명 리스트 가져오기
    ...
    try:    # 보유종목 리스트 호출
        cbid = get_orderi('done', 'bid', 'n', ofn1)                        # 보유종목명 list  
        with open('/root/UBiCauto/data/cp.pickle', 'rb') as fr1:            # mycal 불러오기
            ...
            for b_name in sbl:                              # 검색 종목을 buy_name 에 바인딩
                ...
                if (b_name not in bl) and (b_name not in order_list) and (blcnt < b_count) and (b_name not in bid_w_list) and (mybal >= b_money) :       # 검색종목이 bl에 없고, 매수제한수 이하, 매수대기에 없을 시 
                    # 미적용
                        ...
                        # upbit1.buy_market_order(b_name, b_money)      # (( 시장가 )) 매수
                        # time.sleep(0.2)
                        # sbl.append(b_name)
                    ...
                    if b_name in cu_p2.keys():                                      # 보유종목의 현재가 가져오기
                        ...
                        if b2_bm1[0] and (price < b2_bm1[1]):                       # decline + 매수지연기준   ***
                            ...
                            # b_order = upbit1.buy_limit_order(b_name, b_o, bid_v)  # 지정가 매수 실행 (목표가)(v)
                            b_order = upbit1.buy_limit_order(b_name, price, bid_v)  # 지정가 매수 실행 (현재가)(v)
                            with open('/root/UBiCauto/find.log','a') as fw:
                            ...
                            # 미적용
                                ...
                                # b_order = upbit1.buy_limit_order(b_name, price, bid_v)  # 지정가 매수 실행(현재가) (미적용)
                                # upbit1.buy_market_order(b_name, b_money)                # (( 시장가 )) 매수 미적용
                time.sleep(5)
                bll = gb(upbit1)[0]                 # 보유종목 수량 재호출 위함
    ...
    time.sleep(s_time)
    try:    #매수대기 정보 호출
        bid_w_i = get_orderi('wait', 'bid', 'i', ofn1)                            # 매수 대기목록 정보 리스트
        ...
        for bn in range(len(bid_w_list)):                                   # list 에서 매수대기 종목(dict) 항목 추출
            ...
            if uuids_m in ext_b:
                ...
                if (mt <= datetime.now()) or (uuids_m in bl) or (b_count <= blcnt):
                    b_cancel = upbit1.cancel_order(uuids_id)                    # 매수 대기 취소
