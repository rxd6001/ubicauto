...
from UBiC_Search_def import myasset, ex_item, cnt_p, askper, ticker_per2, one_sell2, perct, de_perct, kwrun, one_buy, linenumber, get_orderi2, t_c2
...
def ubic_reps(user_id):
	...
	if request.method == "POST":
		...
		# print(f'전송방식: {request.method}, cmdh: {cmdh}, cmdb: {cmdb}')
		try:
			if cmdh and cmdb:
		...
		return render_template('/ubic_reps.html', ob_ckw= ob_ck, all_ckw= all_ck, prr_ckw= prr_ck, wrun_ckw= wrun_ck, pbb_ckw= pbb_ck, pss_ckw= pss_ck, myc_ckw= myc_ck)
...
def ubic1(user_id):
	...
	try:
		...
		if user_id == "ubic1":
			...
			ask_p = round((askper(login_upbit(user_id), ex_num) * 100), 2)					# 종목당 매도 기준
		elif user_id == "ubic2":
			ex_num = 2
			ret_ct = ex_item(bl, ex_num)
			ask_p = round((askper(login_upbit(user_id), ex_num) * 100), 2)					# 종목당 매도 기준
		...
		print(ubic1_t)
	# print(add_num)
	keyname1 = ['금일 수익', '이달 수익', '평가 손익', '전체 자산', '여유 금액', '매수 금액', '누적 수익']
	keyname2 = ['수익률(일/월)', '매수 비중']
	
	mycalt ={'금일 수익': format(calsum, ","), '이달 수익':format(msum, ","), '누적 수익': format(allsum, ','), '평가 손익': f'{roep} % / {format(sum_tp, ",")}', '수익률(일/월)': f'{roe} / {troe}', '전체 자산': format(tsum, ","), \
...
def ubic_ear(user_id):
	...
	return render_template("/ubic_ear.html", c_list = c_list)	# mycal
...
def ubic_mear(user_id):
	...
	for i in date_list:
		...
		c_list.append(f)
	print(f'{linenumber()}p::id:{id} --- (1) Cycle time: {round((time.time() - start_time), 3)} seconds ---\n')
	return render_template("/ubic_mear.html", c_list = c_list)	# mycal
...
def ubic_hty(user_id):
	...
	for i in get_hty:
		...
		if '.' in i['price']:
			trs_hty['price'] = float(i['price'])           # 거래단가
		else:
			trs_hty['price'] = '{:,}'.format(int(i['price']))           # 거래단가
		...
		if trs_hty['side'] == 'bid':
			trs_hty['tr_money'] = int(float(i['executed_volume']) * float(i['price']))         # 매수 정산금액
		else:
			trs_hty['tr_money'] = int(float(i['executed_volume']) * float(i['price']) - float(i['paid_fee']))         # 매도 정산금액
