...
import operator
# ========================= 조건 함수 호출 ========================= #
from UBiC_Search_def import r1_m510, bull_market_590, bull_market_o5, bull_market_po, bull_market_pg, bull_market_510, bull_market_1050, \
    bull_market_yc, bull_market_1up, bull_market_3dup, b1_decline_m1
...
from UBiC_Search_def import search_s
...
""" ======================================================================== """
# from UBiC_Rotate_data import cp1, cp2, gb, upbit2
...
while True:
    ...
    with open('/root/UBiCauto/data/ext_item2.pickle', 'rb') as feit:
        ext_item2 = pickle.load(feit)
    ext_b = ext_item2['ext_b']
    ...
    # mybal = round(myasset(upbit2)[3], 1)
    mybal = upbit2.get_balance_t()
    mb1 = math.trunc(mybal / 10000) * 10000
    ...
    with open('/root/UBiCauto/data/set_cfg2.pickle', 'wb') as fw:
    ...
    """ cu_p 제외종목 없이 모두 가져오기 미사용 """    
    # try:
    #     cu_p = cp1()                                # 종목 전체 현재가 호출 후 'cu_p' 로 바인딩
    #     [cu_p.pop(key, None) for key in ext_b]        # 종목 전체 현재가에서 ext 에 있는 종목 제외, key값 None 시 넘어감, 현재 불필요
    # except Exception as cu_p:
    #     print('Buy_cu_p1:', cu_p)
    ...
    try:
        if cu_p:                                                          # 현재가에 값이 있을 때만 실행
            ...
            for ticker, price in cu_p.items():
                # ============================== day == check ================================ #
                """ # --------------------- (안정적) 정렬 기준 값 (적극적) --------------------- # """
                r_tho_1 = rain_market_1ho(ticker)                     # 금일 (고가 - 시가) 차 : 6(%) (금일) (기준 60분: 시가)
                r_y3h3o = rain_market_yho(ticker)                     # 전3일(고가 - 시가) 차 : 25(%) (기준전: 시가)
                r_tho_30 = rain_market_bho(ticker)                    # 금일 (고가 - 시가) 차 : 10(%) (금일) (기준 24시간: 시가)
                r_tpo = rain_market_po(ticker, price)                 # 금일 (현가 - 시가) 차 : 10(%) (기준금 : 시가)
                ...
                # print(f'종목:{ticker}, b_yc:{b_yc}, b_pg:{b_pg}, price:{price}')
    ...
    blcnt = len(bl)                                                               # 거래불가 종목 2개 제외 후 blcnt로 조건문에 수량 적용
    try:                # 종목검색조건 자동 s/w 
        ...
        if (blcnt < b_count) and (len(sbl) == 0) and code == 2:                   # 설정수이하, sbl = 0, code = 2, 수량부족이고 2번일 때 검색 안 됨 => 1로 변경
            ...
            print(f'보유종목수 ≦ 매수가능종목수(설정값), 매수가능수량 = 0, code(2):수량부족 ⇨ ({code}) 변경')
            ...
            print(f'보유종목수 = 매수가능종목수(설정값), code(1):수량충족 ⇨ ({code}) 변경')
            ...
            print(f'보유종목수 = 매수가능종목수(설정값), 매수가능수량 없음, code(2):수량충족 ⇨ ({code}) 변경')
            ...
            print(f'보유종목수 ≪ 매수가능종목수(설정값), 매수가능수량 없음, code(1):수량충족 ⇨ ({code}) 변경')
    ...
    try:
        # bid_w_list = get_order_bid()                    # 매수 대기 종목명 리스트 가져오기
        bid_w_list = get_orderi('wait', 'bid', 'n', ofn2)            # 매수 대기 종목명 리스트 가져오기
    ...
    try:    # 보유종목 리스트 호출
        cbid = get_orderi('done', 'bid', 'n', ofn2)                        # 보유종목명 list  
        with open('/root/UBiCauto/data/cp.pickle', 'rb') as fr1:            # mycal 불러오기
            cu_p2 = pickle.load(fr1)                                       # 전체 종목과 현재가 dict 'cu_p' 로 바인딩
        [cu_p2.pop(key, None) for key in ext_b]                # 종목 전체 현재가에서 ext 에 있는 종목 제외, None 시 넘어감, 현재 불필요
    except Exception as cu_p2:
        print('Buy_cu_p2:', cu_p2)

    try:    # 조건검색 후 신규종목 매수
        if cu_p2:                                           # 현재가에 값이 있을 때만 실행
            for b_name in sbl:                                # 검색 종목을 buy_name 에 바인딩
                ...
                if (b_name not in bl) and (b_name not in order_list) and (blcnt < b_count) and (b_name not in bid_w_list) and (mybal >= b_money) :
                # 미적용 
                    # print(b_name, "신규 매수 대기 시작...")
                    # upbit2.buy_market_order(b_name, b_money)      # (( 시장가 )) 매수
                    # time.sleep(0.2)
                    # sbl.append(b_name)
                    
                    """ (원화마켓 전 종목의 현재가) 가져오기 """
                    if b_name in cu_p2.keys():                                        # 보유종목의 현재가 가져오기
                        price = cu_p2[b_name]                                         # 해당종목(b_name)의 현개가를 price 에 바인딩
                        b1_bm1 = b1_decline_m1(b_name)                                # 매수지연 조건
                        # gr_m510 = r3_m510(b_name, price)                            # 매수조건 계산(3m)
                        # gr_m510 = r1_m510(b_name, price)                            # 매수조건 계산(1m)
                        # if True == gr_m510:                                         # 조건 == True 일 경우 매수 대기
                        if b1_bm1[0] and (price < b1_bm1[1]):                         # decline + 매수지연기준
                            pass
                            """ ===================== 지정가 매수 주문 ===================== """
                        else:
                            bid_p = price - (price * bg)                          # 매수 목표가 계산 ( 현재가의 - 1.5 %)(v)
                            b_o = pyupbit.get_tick_size(bid_p)                    # 매수 호가 계산(v)
                            bid_v = b_money / bid_p                                 # 매수 수량 계산(목표가) (매수금액 기준)(v)
                            # b_order = upbit2.buy_limit_order(b_name, b_o, bid_v)  # 지정가 매수 실행 (목표가)(v)
                            b_order = upbit2.buy_limit_order(b_name, price, bid_v)  # 지정가 매수 실행 (목표가)(v)

                            with open('/root/UBiCauto/find2.log','a') as fw:
                                print(f'{now}: 종목:{b_name}, 매수가:{price}', file=fw)

                            order_list.append(b_name)
                            time.sleep(5)
                            # 미적용
                                # b_o = pyupbit.get_tick_size(price)                      # 매수 호가 계산 (o)
                                # bid_v = b_money / price                                 # 매수 수량 계산(현재가) (매수금액 기준) (o)
                                # b_order = upbit2.buy_limit_order(b_name, price, bid_v)  # 지정가 매수 실행(현재가) (o)
                                # upbit2.buy_market_order(b_name, b_money)              # (( 시장가 )) 매수
                time.sleep(5)
                bll = gb(upbit2)[0]                 # 보유종목 수량 재호출 위함
                bllcnt = len(bll)                   # 매도 불가종목 제외
                print(f'보유종목수: {blcnt}, 보유종목수(매도제외수량적용): {bllcnt}')
    ...
    time.sleep(s_time)
    try:    #매수대기 정보 호출
        bid_w_i = get_orderi('wait', 'bid', 'i', ofn2)                            # 매수 대기목록 정보 리스트
    except Exception as wbid_i:
        print('Buy_wbid:', wbid_i)
        ...
        for bn in range(len(bid_w_list)):                                   # list 에서 매수대기 종목(dict) 항목 추출
            ...
            if uuids_m in ext_b:
                ...
                if (mt <= datetime.now()) or (uuids_m in bl) or (b_count <= blcnt):
                    b_cancel = upbit2.cancel_order(uuids_id)                    # 매수 대기 취소
