...
b_money = 7100                  # 0.75 % 시 7100원 부터 매수가능함  10,000 초기 매수 금액 설정 (금액이 적어지면 신규매수 시 add_num에 못들어가서 1차 추매 시 "nonetype error")

# ========================= b_money 계산 ======================== #
bp =  4                         # 보유금액의 % 매수(개당 매수금액 계산필요)
b_count = 25                     # 보유 수량 제한 설정
blow_money = 7100               # 최저 매수금액
# ============================================================== #
bg = perct(0.1)                 # 매수예상 지정가 < 현재가 bg = - 0.5% 에 매수 실행, 현재 3m으로 현재가 매수대기 들어감(ex - 0.01 --> 1%)
gt = 25                         # 매수대기 종목 취소 시간 ( 초, 분(*) )
code = 1                        # 1:기본 상승장, 2:여러종목 검색(3분기준)
""" --------------------------------------------------------------------------------------------- """
...
npct = perct(20)                # nw이평에서 npct % 이상일 경우 매수(매수 하한)

yg = 60                         # 전일(yg이평 < 종가) 첫번째 기준 보다 넓게

""" ========== 제외할 종목 ========== """
ext_b = ['KRW-VTHO', 'KRW-XYM']
ext_s = ['KRW-VTHO', 'KRW-XYM']
# ext_b = []
# ext_s = []
""" =============================== """
...
""" --------------- """
l_money = 120000                                  # 추매 5회까지 가능 / 금액 제한 (종목당 보유금액( 보유금액의 2배 까지 증가 가능)
""" --------------- """

""" --------------------------------------------------------------------------------------------- """
ag1 = 1                     # 추매 금액 (종목 보유금액의 1배)
ag2 = 0.75                   # 추매 금액 (종목 보유금액의 0.75배)
ag3 = 2                     # 추매 금액 (종목 보유금액의 2배)
ag4 = 2.5                   # 추매 금액 (종목 보유금액의 2.5배)
...
with open('/root/UBiCauto/data/config_sell3.pickle', 'wb') as fsell:
    ...
    pickle.dump(ag1, fsell); pickle.dump(ag2, fsell); pickle.dump(ag3, fsell); pickle.dump(ag4, fsell)
    pickle.dump(ag5, fsell); pickle.dump(abptime, fsell); pickle.dump(ext_s, fsell)
    pickle.dump(ofn3, fsell); pickle.dump(ask_ug, fsell); pickle.dump(adsp, fsell); pickle.dump(blow_money, fsell)
