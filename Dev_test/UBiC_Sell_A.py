...
if os.path.isfile('/root/UBiCauto/data/add_num1.pickle'):
    with open('/root/UBiCauto/data/add_num1.pickle', 'rb') as fr:
        add_num = pickle.load(fr)

now = datetime.now()    # 현재시간 호출
rwnum = 1           # add_num에 필요한 변수
ex_num = 1          # add_num 종목제외 시 필요(ex_item(bl, ex_num))
...
while True:
    ...
    with open('/root/UBiCauto/data/ext_item1.pickle', 'rb') as fset:
    ...
    ofn1 = '/root/UBiCauto/Acct/upbit1.txt'
     
    t_tp = []                                             # 평가손익(리스트)

    try:
        bl, abp1, tb1 = gb(upbit1)                              # 보유종목명, 평단, 보유수량 호출                       
        tsum, tbidm, tbidp, summ = myasset(upbit1)              # 자산정보 한 번에 호출
        summ = round(summ, 1)
    ...
    for e in bl:                                            # 자산('bl') 리스트를 하나씩 'e'로 가져 옴
        if abp_check(e, abptime, ofn1):                           # 매수 평단 선택 (체크)
            abp = abp_check(e, abptime, ofn1)                     # 매수평단 최근 매수완료 정보에서 가져 옴
        else:
        ...
        """ E ~~~ 추매시점 지정 %(adsp %) 이하 시 (소수 6자리에서 반올림) """
        addprice = round((abp - (abp * adsp)), 6)             # 매수 시 매수평단의 adsp % 이하 일 경우 추매 진행 (평단가)
        ...
        if (e in cu_p):                                     # 한 종목에 대한 현재가 추출
            ...
            if t_money <= 5000:     # 매도 후 잔금이 5000원 이하 남을 시 추매 (0.01 -> 0.1로 변경 3/15)
                ...
                upbit1.buy_market_order(e, bid_money)   # 현매수금액과 차액만큼 매수
                add_rw0(e, rwnum)                  # 매수 후 애드넘 횟수 초기화(0번)
                
                with open('/root/UBiCauto/find_s1.log','a') as fw:
                    print(f'{now}: (line-112) 종목:{e}, 총계: {t_money}, 매수금액:{ag_price1}, 추매수:{add_num[e][0:1]}, tb: {tb}, abp:{abp}', file=fw)
                # 미적용
                    # elif (0.01 < t_money) and (price <= (abp - (abp * perct(2)))) and (t_money <= (b_money - 1000)):     # 종목보유금액 < 매수금액(-1000) , (price < (abp - (abp * perct(1))) and
                    #     g_money = b_money - t_money                         # 목표가 = 매수금액 - 보유금액
                    #     if 5000 < g_money:
                    #         upbit1.buy_market_order(e, g_money)
                    #     elif g_money <= 5000:
                    #         g1_money = (5000 - g_money) + 500
                    #         g_money = (g_money + g1_money)
                    #         upbit1.buy_market_order(e, g_money)
                    # print(e, "--- %s seconds ---" % round((time.time() - start_time), 3))
                    # print(sell_cont)
            ...
            if sell_cont == 0:                                      # 전체 매도 제외, 전체 추매 제외
                ...
                if (sell_cont == 1) or (sell_cont == 2):            # 매도기능
                    if s1_bm1 and (s1_bm2 < price):                 # bulling + 매도기준 만족 시
                        ...
                        if e in ext_s:
                            ...
                            a_order = upbit1.sell_limit_order(e, price, tb)       # '지정가 < 현재가' 조건 시 현재가 매도 실행
                            
                            # 미적용
                                # print(j, "판매완료")                        
                                # upbit1.sell_market_order(e, tb)  # '종목' 을 시장가로 매도
                                # a_order = upbit1.sell_limit_order(e, a_o, tb)       # '지정가 < 현재가' 조건 시 지정가 매도 실행
                ...
                if (sell_cont == 1) or (sell_cont == 3):          # 추매기능
                    if e in ext_a:
                        ...
                        if e not in add_num :
                            cbid = get_orderi('done', 'bid', 'i', ofn1)         # 매수완료 정보 가져오기
                            for i in cbid:
                                if e == i['market']:
                        ...
                        an = add_num.get(e)[0]         # {'보유종목': [매수횟수, 매수시점]}, 보유종목의 매수 횟수를 'an' 에 바인딩
                        ...
                        if b2_bm1 and (price < b2_bm2):           # decline + 추매지연기준 2    
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                upbit1.buy_market_order(e, ag_price1)   # 0.7배 추매
                                ...
                                upbit1.buy_market_order(e, ag_price1)
                                if 5000 <= addprice_1:
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                ...
                                upbit1.buy_market_order(e, ag_price2)   # 1배 추매
                                if 5000 <= addprice_2:
                                    add_rwav5(an, e, rwnum)

                            elif (buy_goal10[1] < abp) and (price < buy_goal20[0]) and (t_money < l_money):
                            
                                upbit1.buy_market_order(e, ag_price2)
                                if 5000 <= addprice_2:
                                    add_rwav10(an, e, rwnum)
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                ...
                                upbit1.buy_market_order(e, ag_price3)   # 1배 추매
                                if 5000 <= addprice_3:
                                    add_rwav5(an, e, rwnum)

                            elif (buy_goal10[1] < abp) and (price < buy_goal20[0]) and (t_money < l_money):
                            
                                upbit1.buy_market_order(e, ag_price3)
                                if 5000 <= addprice_3:
                                    add_rwav10(an, e, rwnum)
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                ...
                                upbit1.buy_market_order(e, ag_price4)   # ** 1.5배 추매 **
                                if 5000 <= addprice_4:
                                    add_rwav5(an, e, rwnum)
                                ...
                                upbit1.buy_market_order(e, ag_price4)
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                ...
                                upbit1.buy_market_order(e, ag_price5)   # ** 2배 추매 **
                                if 5000 <= addprice_5:
                                    add_rwav5(an, e, rwnum)
                                ...
                                upbit1.buy_market_order(e, ag_price5)
                                if 5000 <= addprice_5:
                                    add_rwav50(an, e, rwnum)
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                ...
                                upbit1.buy_market_order(e, ag_price6)   # 1배 추매 **
                                if 5000 <= addprice_6:
                                    add_rwav5(an, e, rwnum)
                                ...
                                upbit1.buy_market_order(e, ag_price6)
                                if 5000 <= addprice_6:
                                    add_rwav60(an, e, rwnum)
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                ...
                                upbit1.buy_market_order(e, ag_price7)   # 1배 추매 **
                                if 5000 <= addprice_7:
                                    add_rwav5(an, e, rwnum)
                                ...
                                upbit1.buy_market_order(e, ag_price7)
                                if 5000 <= addprice_7:
                                    add_rwav70(an, e, rwnum)
                            ...
                            if (buy_goal5[1] < abp) and (price < buy_goal10[0]) and (t_money < l_money):
                                ...
                                upbit1.buy_market_order(e, ag_price8)   # 1배 추매 **
                                if 5000 <= addprice_8:
                                    add_rwav5(an, e, rwnum)
                                ...
                                upbit1.buy_market_order(e, ag_price8)
                                if 5000 <= addprice_8:
                                    add_rwav80(an, e, rwnum)
    ...
    with open('/root/UBiCauto/data/t_tp1.pickle', 'wb') as fw:    # 보유종목 평가손익(list 로 저장)
        pickle.dump(t_tp, fw)
    ...
    ask_p = askper(upbit1, 1)                   # 매도기준 조정 및 저장
    ...
    try:    # 매도대기 취소
        ask_w_i = get_orderi('wait', 'ask', 'i', ofn1)                                # 매도대기 목록 정보 리스트
        ask_w_list = get_orderi('wait', 'ask', 'n', ofn1)                             # 매도대기 종목 리스트

        for an in range(len(ask_w_list)):                               # list 에서 매도대기 종목(dict) 항목 추출
            uuids_an = ask_w_i[an]['market']                            # 매도대기 종목 정보에서 가져 온 종목이름
            if (uuids_an in ext_s) or ("KRW-BTT" == uuids_an) or ("KRW-ZRX" == uuids_an):             # 매도제외 종목에 대기종목이 있을 경우 또는 KRW-BTT 
                pass
            else:
                ...
                if (mt <= datetime.now()):                                  # 지정시간이 지났는지, 자산목록에 종목이 있는지 확인 후
                    b_cancel = upbit1.cancel_order(uuids_aid)               # 매도 대기 취소
    except Exception as s3:
        print('e-time:', datetime.now())
        print('s3_매도대기', s3)

    time.sleep(1)
    if os.path.isfile('/root/UBiCauto/data/add_num1.pickle') and os.path.getsize('/root/UBiCauto/data/add_num1.pickle') > 5:
        os.system('clear')
